...
void HTTPFileServer::_ev_handler ( mg_connection* c, const int ev, void* ev_data ) {
	...
	if ( ev == MG_EV_HTTP_MSG ) {
		...
		if ( strcmp(buf, "yes") == 0 ) {
			static const auto header = "Content-Disposition: filename=\"" + filePath + "\"\r\n";
		...
		if ( strcmp(buf, "no") == 0 ) {
			static const auto download_header = std::string("Content-Disposition: attachment; filename=\"") + filePath + "\"\r\n";
